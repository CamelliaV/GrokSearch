...
4. **Maximize parallel searches** — When multiple independent queries are needed, use the `queries` parameter to batch them in a single call. This ensures true parallel execution at the server level. Otherwise, execute as many parallel searches as possible to broaden coverage and enable cross-verification.
